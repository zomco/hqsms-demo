<template>
  <div>
    ...
    <el-card>
      ...
      <el-row :gutter="20">
        <el-col :span="8">
          <el-input placeholder="请输入报警器id"
                    v-model="id"
                    clearable
                    @clear="getAlarmLogList">
            <el-button slot="append"
                       icon="el-icon-search"
                       @click="queryAlarmLogById"></el-button>
          </el-input>
        </el-col>
        <el-col :span="4">
...
export default {
  ...
  methods: {
    ...
    async getAlarmLogList () {
      const res = await this.$http.get('alarm-logs')
      if (res.status !== 200) {
        this.$message.error('数据请求失败')
      }
      this.$message.success('请求数据成功')
      this.AlarmLogList = res.data.content
      this.total = res.data.totalPages
    },
    // 通过id查报警器
    async queryAlarmLogById () {
      const res = await this.$http.get(`alarm-logs/${this.id}`);
      if (res.status !== 200) {
        this.$message.error("查询失败");
      }
      console.log(res);
      this.AlarmLogList = [];
      this.AlarmLogList.push(res.data);
    },
